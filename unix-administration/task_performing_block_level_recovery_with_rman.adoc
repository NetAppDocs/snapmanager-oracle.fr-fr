...
summary: 'Vous pouvez configurer SnapManager pour cataloguer ses sauvegardes dans le cadre de restauration \(RMAN\), un outil Oracle, de manière à pouvoir effectuer une restauration au niveau des blocs à l"aide de RMAN. RMAN peut utiliser les fichiers de contrôle de la base de données ou une base de données de catalogue de récupération distincte comme référentiel.' 
---
...
. Pour effectuer une sauvegarde entièrement hors ligne à l'aide de SnapManager, entrez la commande suivante :
+
...
+
----
. Pour vérifier que la sauvegarde est cataloguée avec RMAN, à partir de l'hôte de la base de données, entrez la commande suivante à l'invite RMAN :
+
...
using target database control file instead of recovery catalog
 ...
 RMAN>
----
. Pour vérifier la base de données et déterminer si des blocs sont corrompus, entrez la commande suivante :
+
dbv FILE=user01.dbf
...
----
. Pour rendre les fichiers de la sauvegarde accessible sur l'hôte et sur RMAN, montez la sauvegarde à l'aide de la commande suivante :
+
...
----
. Pour restaurer les blocs, dans RMAN, entrez la commande suivante :
+
...
----
. Pour vérifier si les blocs ont été réparés, utilisez la commande suivante :
+
